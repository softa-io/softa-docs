...
主要能力：
- **模型设计器**：设计 Model、Field、OptionSet、View、Navigation、Validation、Onchange 规则、Index 以及生成映射/模板
...
- **部署**：Version 到环境的直接部署，通过 `sealedTime` 自动合并已发布版本、生成并执行 DDL、跟踪执行；新环境会合并到目标版本为止的所有已发布版本
- **多环境部署**：通过签名的远程通道部署到 Dev/Test/UAT/Prod
- **漂移检测与运行时导入**：按环境将设计时快照与线上运行时对比，并可选地以运行时状态覆盖设计时元数据——既覆盖首次从已有运行时拉取元数据（设计时为空、运行时已有数据），也覆盖越权 SQL 之后的漂移修复
...
## 远程部署配置
- `DesignAppEnv.upgradeEndpoint` 必须指向目标运行时的基 URL。Studio 会自动追加 `/metadata/upgrade` 和 `/metadata/exportRuntimeMetadata`。
...
| `DesignDeploymentVersion` | 审计：将一次部署与参与合并的多个版本关联 |
...
运行时导出（`/metadata/exportRuntimeMetadata`）按应用范围：Studio 传入环境的 `appId`，主模型按 `appId` 列过滤，翻译类模型通过父行 join；单运行时托管多应用时，不会将兄弟应用数据泄漏到漂移比较或导入中。
...
4. 创建 **Version**（DRAFT），`versionType = Normal | Hotfix` → 加入已完成的 WorkItem → `sealVersion`
   - 封版会聚合 WorkItem 变更、计算 diffHash、转为 SEALED
   - 若尚未部署，可用 `unsealVersion` 将 SEALED 版本退回 DRAFT
5. 部署已发布版本
...
| `POST /DesignWorkItem/reopenWorkItem?id=` | 重新打开已完成/已取消/已延期的工作项 |
...
| 端点 | 说明 |
| --- | --- |
| `GET /DesignAppEnv/compareDesignWithRuntime?id=` | 读取 `DesignAppEnvSnapshot` 与运行时元数据之间的缓存漂移 |
