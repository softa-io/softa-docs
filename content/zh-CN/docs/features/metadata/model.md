...
### 2.7 `tableName` 表名
只读字段，模型对应的数据库表名，从模型名自动转换，如 `product_category`。模型重命名时表名随之同步——请声明重命名（`renamedFrom`），物理表才会原地改名并携带数据。DDL 是否实际执行取决于运行时姿态：开发环境由启动扫描器的 `scanner-scope` 决定，生产环境走 Studio 发布流程。
