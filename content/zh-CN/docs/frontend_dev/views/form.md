...
- **Versions 面板**：时间轴模型的已持久化记录上，`FormBody` 在审计日志上方渲染 `Versions` 侧板（同一右栏 / 底部堆叠）。每行显示生效区间、Current / Past / Future 徽标与摘要（`timeline.versionSummaryFields`，缺省用模型 `displayName`）；当前加载版本高亮。行动作：`Open` / `Correct` 经 `?sliceId=` 搜索参数导航（只读 / 编辑模式；侧滑表单壳内隐藏——无切片路由），`Delete` 在破坏性确认后删除该版本。`Correct` 与 `Delete` 遵循工具栏权限约定（update / delete 状态；无权限时隐藏，未纳管模型显示）。删除**唯一**版本即删除实体本身（时间轴实体就是它的全部切片），确认文案升级为记录级措辞并改走实体删除——`onDelete` 外键删除策略随之生效（侧滑表单壳内唯一版本的删除保持禁用：无列表导航目标）。展开行加载该版本的切片级变更日志（`getSliceChangeLog`）。行按最新在前排序；不连续的版本之间渲染一条虚线分隔 `No coverage: <start> → <end>`——空档在后端是合法状态（落在空档内的 as-of 读取返回空），分隔条让它显式可见而非默默隐含。用 `timeline.enableVersionPanel: false` 关闭面板。
- **终结 / 重开（Terminate / Reopen）**：尾部（最新）版本行带 `Terminate…` 动作——对话框询问结束日期（默认今天，不得早于尾版本的起始日期；要更早终结，先删除尾部版本，结束日期会按后端愈合规则向前转移）。确认后调用 `setEndDate(id, endDate)`。已终结的尾版本显示破坏性色调的 `Terminated` 徽标，动作翻转为 `Reopen`——确认后调用 `setEndDate(id, "9999-12-31")`，记录恢复开放式有效期。两者都由 update 权限状态门控（与 `Correct` 同一约定），且在嵌入式宿主中保持可用（仅变更数据、不导航）。注意对话框的过去日期警告：用早于今天的日期终结会让记录立即从所有 as-of 列表与详情页消失——只能通过 Versions 面板的 `Open` 链接（`?sliceId=`）访问，重开也在那里完成。
