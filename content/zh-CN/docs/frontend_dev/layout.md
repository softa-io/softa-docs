...
应用外壳用的客户端组件：顶栏、侧栏、工作区切换、全局页面浏览器，以及路由级标签。它们位于业务页面之上，与 `@/navigation` 清单和工作区上下文集成——**不**依赖模型元数据（后者在 `src/components/views/`）。

| 文件 | 导出 | 作用 |
|------|--------|------|
| `header.tsx` | `Header` | 顶栏壳层：模块切换入口、面包屑、搜索入口、用户菜单、按需加载的对话框 |
| `sidebar.tsx` | `Sidebar` | 由当前模块清单生成的可折叠导航；模块声明了 workspace 时内嵌 `WorkspaceSwitcher` |
| `WorkspaceSwitcher.tsx` | `WorkspaceSwitcher` | 可搜索的工作区记录选择器（如应用/租户），在活跃模块的导航清单含 `workspace` 时启用 |
| `browse-pages-dialog.tsx` | `BrowsePagesDialog` | 全屏式对话框，跨模块浏览并跳转页面（从顶栏打开） |

导入路径使用 `@/components/layout/...` 别名，例如 `@/components/layout/header`。

单个页面内的多标签视图（多个列表或多种视图），请参阅 [MultiView](./views/multi-view)。
...
- 高度为 `var(--ui-shell-header-height)`，水平内边距为 `var(--ui-page-padding)`。
- **模块 / 浏览：** 打开 `BrowsePagesDialog`（代码分割）。存在多个模块时，触发器旁显示当前模块文案。
...
- 否则渲染组合框式浮层：通过 `modelService` 加载工作区记录，支持搜索、选择，以及在 `module.workspace.clearable` 允许时清空。
