...
| `enableCreate`| `boolean`                                                     | No       | `true`  | Show Create button in toolbar. Forced off on a projection model (`MetaModel.projection === true` — read-only model over a table another model owns). |
| `enableDelete`| `boolean`                                                     | No       | `false` | Show `...` delete action on each card. Forced off on a projection model, same as `enableCreate`.             |
